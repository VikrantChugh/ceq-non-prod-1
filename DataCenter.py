...
def fetch_oci_datacenters(signer):
    # details.logger.info("start fetching details from cmdb_ci_cloud_datacenter")
    datacenters = []
    try:
        ...
        for subscribed_region in subscribed_regions:
            ...
            for compartment in active_compartments:
                # details.logger.info(f"start fetching {table_name} details from account -> {compartment['compartment_name']} and region is ->{region_name}")
                datacenters.append({
                    'name' : region_name,
                    'region': region_name,
                    'object_id' : region_name,
